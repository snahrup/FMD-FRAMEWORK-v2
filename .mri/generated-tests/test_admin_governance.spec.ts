...
/**
 * Comprehensive tests for: /admin-governance
 * Component: AdminGovernance
 * Source: dashboard/app/src/pages/AdminGovernance.tsx
 *
 * KPIs detected: entities, Source, pending
 * Table columns: Connections, Landing Zone, Bronze, Silver, Source, Landing, Gold, Bronze Layer
 *
 * Generated by MRI Test Generator
 */

test.describe('Page: /admin-governance', () => {
  ...
  test('all safe buttons are clickable without crash', async ({ page }) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      await expect(page.locator('body')).toBeVisible();
      if (!page.url().includes('/admin-governance')) {
        await page.goto('/admin-governance');
        await waitForStable(page);
      }
    }
  ...
  test('renders without crash at mobile width (375px)', async ({ page }) => {
    ...
    const bodyHTML = await page.locator('body').innerHTML();
    expect(bodyHTML.trim().length).toBeGreaterThan(50);
    await screenshot(page, 'admin_governance-mobile');
  });
  ...
  test('renders without crash at ultrawide width (2560px)', async ({ page }) => {
    ...
    const bodyHTML = await page.locator('body').innerHTML();
    expect(bodyHTML.trim().length).toBeGreaterThan(50);
    await screenshot(page, 'admin_governance-ultrawide');
  });
  ...
  test('captures full-page screenshot', async ({ page }) => {
    await navigateToPage(page);
    await screenshot(page, 'admin_governance');
  });
});
