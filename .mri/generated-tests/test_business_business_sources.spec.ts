...
/**
 * Comprehensive tests for: /business/business-sources
 * Component: BusinessSources
 * Source: dashboard/app/src/pages/business/BusinessSources.tsx
 *
 * KPIs detected: tables in scope, Table, Source, Tables that made it into Fabric landing, Tables that successfully materialized in bronze, Sources Healthy, Landing Loaded, Bronze Loaded
 * Status filters: all
 * Has search: yes
 * Table columns: What This Page Is, Why It Matters, What Happens Next, Open Overview, Inspect Data Estate, Open Source Manager
 *
 * Generated by MRI Test Generator
 */

test.describe('Page: /business/business-sources', () => {

  async function navigateToPage(page: import('@playwright/test').Page) {
    await page.goto('/business/business-sources');
  ...
  test('all safe buttons are clickable without crash', async ({ page }) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      await expect(page.locator('body')).toBeVisible();
      if (!page.url().includes('/business/business-sources')) {
        await page.goto('/business/business-sources');
        await waitForStable(page);
      }
    }
  ...
  test('renders without crash at mobile width (375px)', async ({ page }) => {
    ...
    const bodyHTML = await page.locator('body').innerHTML();
    expect(bodyHTML.trim().length).toBeGreaterThan(50);
    await screenshot(page, 'business_business_sources-mobile');
  });
  ...
  test('renders without crash at ultrawide width (2560px)', async ({ page }) => {
    ...
    const bodyHTML = await page.locator('body').innerHTML();
    expect(bodyHTML.trim().length).toBeGreaterThan(50);
    await screenshot(page, 'business_business_sources-ultrawide');
  });
  ...
  test('captures full-page screenshot', async ({ page }) => {
    await navigateToPage(page);
    await screenshot(page, 'business_business_sources');
  });
});
