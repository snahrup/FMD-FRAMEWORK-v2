...
/**
 * Comprehensive tests for: /business/dataset-detail
 * Component: DatasetDetail
 * Source: dashboard/app/src/pages/business/DatasetDetail.tsx
 *
 * KPIs detected: Freshness, Quality
 * Table columns: What This Page Is, Why It Matters, What Happens Next, Return to Catalog, Submit a Request, Open Help, Browse Sources
 *
 * Generated by MRI Test Generator
 */

test.describe('Page: /business/dataset-detail', () => {

  async function navigateToPage(page: import('@playwright/test').Page) {
    await page.goto('/business/dataset-detail');
  ...
  test('all safe buttons are clickable without crash', async ({ page }) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      await expect(page.locator('body')).toBeVisible();
      if (!page.url().includes('/business/dataset-detail')) {
        await page.goto('/business/dataset-detail');
        await waitForStable(page);
      }
    }
  ...
  test('renders without crash at mobile width (375px)', async ({ page }) => {
    ...
    const bodyHTML = await page.locator('body').innerHTML();
    expect(bodyHTML.trim().length).toBeGreaterThan(50);
    await screenshot(page, 'business_dataset_detail-mobile');
  });
  ...
  test('renders without crash at ultrawide width (2560px)', async ({ page }) => {
    ...
    const bodyHTML = await page.locator('body').innerHTML();
    expect(bodyHTML.trim().length).toBeGreaterThan(50);
    await screenshot(page, 'business_dataset_detail-ultrawide');
  });
  ...
  test('captures full-page screenshot', async ({ page }) => {
    await navigateToPage(page);
    await screenshot(page, 'business_dataset_detail');
  });
});
