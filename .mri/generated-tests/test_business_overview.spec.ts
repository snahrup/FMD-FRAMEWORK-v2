...
/**
 * Comprehensive tests for: /business-overview
 * Component: BusinessOverview
 * Source: dashboard/app/src/pages/BusinessOverview.tsx
 *
 * KPIs detected: Landing Loaded, Silver Loaded, Tool-Ready, Quality Avg, Source Health
 * Table columns: Load Center, Mission Control, Explore, Gold Studio
 *
 * Generated by MRI Test Generator
 */

test.describe('Page: /business-overview', () => {
  ...
  test('all safe buttons are clickable without crash', async ({ page }) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      await expect(page.locator('body')).toBeVisible();
      if (!page.url().includes('/business-overview')) {
        await page.goto('/business-overview');
        await waitForStable(page);
      }
    }
  ...
  test('renders without crash at mobile width (375px)', async ({ page }) => {
    ...
    const bodyHTML = await page.locator('body').innerHTML();
    expect(bodyHTML.trim().length).toBeGreaterThan(50);
    await screenshot(page, 'business_overview-mobile');
  });
  ...
  test('renders without crash at ultrawide width (2560px)', async ({ page }) => {
    ...
    const bodyHTML = await page.locator('body').innerHTML();
    expect(bodyHTML.trim().length).toBeGreaterThan(50);
    await screenshot(page, 'business_overview-ultrawide');
  });
  ...
  test('captures full-page screenshot', async ({ page }) => {
    await navigateToPage(page);
    await screenshot(page, 'business_overview');
  });
});
