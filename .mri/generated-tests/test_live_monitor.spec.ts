...
/**
 * Comprehensive tests for: /live-monitor
 * Component: LiveMonitor
 * Source: dashboard/app/src/pages/LiveMonitor.tsx
 *
 * KPIs detected: py-3 flex items-center gap-2 text-sm, pb-2 cursor-pointer, text-base flex items-center gap-2, pt-0, space-y-5 pt-0
 *
 * Generated by MRI Test Generator
 */

test.describe('Page: /live-monitor', () => {
  ...
  test('all safe buttons are clickable without crash', async ({ page }) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      await expect(page.locator('body')).toBeVisible();
      if (!page.url().includes('/live-monitor')) {
        await page.goto('/live-monitor');
        await waitForStable(page);
      }
    }
  ...
  test('renders without crash at mobile width (375px)', async ({ page }) => {
    ...
    const bodyHTML = await page.locator('body').innerHTML();
    expect(bodyHTML.trim().length).toBeGreaterThan(50);
    await screenshot(page, 'live_monitor-mobile');
  });
  ...
  test('renders without crash at ultrawide width (2560px)', async ({ page }) => {
    ...
    const bodyHTML = await page.locator('body').innerHTML();
    expect(bodyHTML.trim().length).toBeGreaterThan(50);
    await screenshot(page, 'live_monitor-ultrawide');
  });
  ...
  test('captures full-page screenshot', async ({ page }) => {
    await navigateToPage(page);
    await screenshot(page, 'live_monitor');
  });
});
