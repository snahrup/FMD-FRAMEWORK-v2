...
/**
 * Comprehensive tests for: /notebook-debug
 * Component: NotebookDebug
 * Source: dashboard/app/src/App.tsx
 *
 * KPIs detected: w-4 h-4 shrink-0, w-4 h-4 animate-spin shrink-0, Tables will be processed in batches of 50., Running pipeline test..., Running..., Tables Included in Test, Source, Table
 * Table columns: #, Source, Schema, Table, File, Extract from Source, Load to Bronze, Transform to Silver
 *
 * Generated by MRI Test Generator
 */

test.describe('Page: /notebook-debug', () => {
  ...
  test('all safe buttons are clickable without crash', async ({ page }) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      await expect(page.locator('body')).toBeVisible();
      if (!page.url().includes('/notebook-debug')) {
        await page.goto('/notebook-debug');
        await waitForStable(page);
      }
    }
  ...
  test('renders without crash at mobile width (375px)', async ({ page }) => {
    ...
    const bodyHTML = await page.locator('body').innerHTML();
    expect(bodyHTML.trim().length).toBeGreaterThan(50);
    await screenshot(page, 'notebook_debug-mobile');
  });
  ...
  test('renders without crash at ultrawide width (2560px)', async ({ page }) => {
    ...
    const bodyHTML = await page.locator('body').innerHTML();
    expect(bodyHTML.trim().length).toBeGreaterThan(50);
    await screenshot(page, 'notebook_debug-ultrawide');
  });
  ...
  test('captures full-page screenshot', async ({ page }) => {
    await navigateToPage(page);
    await screenshot(page, 'notebook_debug');
  });
});
