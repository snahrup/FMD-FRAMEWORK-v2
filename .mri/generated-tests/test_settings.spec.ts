 ...
 *
 * KPIs detected: Active
 * Has search: yes
 * Table columns: Cleansing Rule Editor, SCD Audit View, Gold Layer / MLV Manager, DQ Scorecard, Config Workspace ID, Fabric SQL Connection GUID, Enable Lakehouse Schemas, General
 *
...
test.describe('Page: /settings', () => {
  ...
  test('renders without crash at ultrawide width (2560px)', async ({ page }) => {
    ...
    const bodyHTML = await page.locator('body').innerHTML();
    expect(bodyHTML.trim().length).toBeGreaterThan(50);
    await screenshot(page, 'settings-ultrawide');
  });
