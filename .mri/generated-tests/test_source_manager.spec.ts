...
/**
 * Comprehensive tests for: /source-manager
 * Component: SourceManager
 * Source: dashboard/app/src/pages/SourceManager.tsx
 *
...
test.describe('Page: /source-manager', () => {
  ...
  test('status filters toggle correctly and update displayed data', async ({ page }) => {
    await navigateToPage(page);
    const btn_incremental = page.locator('button:visible, [role="tab"]:visible').filter({ hasText: /incremental/i }).first();
    if (await btn_incremental.isVisible({ timeout: 1000 }).catch(() => false)) {
      await btn_incremental.click();
      await page.waitForTimeout(500);
      await expect(page.locator('body')).toBeVisible();
    }
    const btn_full = page.locator('button:visible, [role="tab"]:visible').filter({ hasText: /full/i }).first();
    if (await btn_full.isVisible({ timeout: 1000 }).catch(() => false)) {
      await btn_full.click();
      await page.waitForTimeout(500);
      await expect(page.locator('body')).toBeVisible();
    }
  ...
  test('clicking same filter twice toggles it off', async ({ page }) => {
    await navigateToPage(page);
    const rowsBefore = await page.locator('tbody tr').count();
    const filterBtn = page.locator('button:visible').filter({ hasText: /incremental/i }).first();
  ...
  test('all safe buttons are clickable without crash', async ({ page }) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      await expect(page.locator('body')).toBeVisible();
      if (!page.url().includes('/source-manager')) {
        await page.goto('/source-manager');
        await waitForStable(page);
      }
    }
  ...
  test('renders without crash at mobile width (375px)', async ({ page }) => {
    ...
    const bodyHTML = await page.locator('body').innerHTML();
    expect(bodyHTML.trim().length).toBeGreaterThan(50);
    await screenshot(page, 'source_manager-mobile');
  });
  ...
  test('renders without crash at ultrawide width (2560px)', async ({ page }) => {
    ...
    const bodyHTML = await page.locator('body').innerHTML();
    expect(bodyHTML.trim().length).toBeGreaterThan(50);
    await screenshot(page, 'source_manager-ultrawide');
  });
  ...
  test('captures full-page screenshot', async ({ page }) => {
    await navigateToPage(page);
    await screenshot(page, 'source_manager');
  });
});
