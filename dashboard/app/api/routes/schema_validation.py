    ...
    try:
        rows = conn.execute("""
            SELECT sv.id, sv.entity_id, sv.layer, sv.passed, sv.schema_name,
                   ...
                   lz.SourceName, lz.SourceSchema, d.Name AS DataSourceName
            FROM schema_validations sv
            LEFT JOIN lz_entities lz ON sv.entity_id = lz.LandingzoneEntityId
            LEFT JOIN datasources d ON d.DataSourceId = lz.DataSourceId
            WHERE sv.run_id = ?
...
def get_entity_history(params: dict):
    ...
    try:
        validated = conn.execute("""
            SELECT
                sv.entity_id,
                lz.SourceName,
                COALESCE(d.Name, d.Namespace, 'Unknown') AS source_database,
                lz.SourceSchema,
            ...
            GROUP BY sv.entity_id, lz.SourceName, source_database, lz.SourceSchema
        """).fetchall()
