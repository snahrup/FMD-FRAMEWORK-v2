...
class LoadOrchestrator:
    ...
    def run(
        ...
        force_full_refresh : bool
            For a scoped proof/backfill run, ignore persisted landing watermarks
            and execute the selected entities as full loads for this run only.
        ...
        try:
            ...
            if force_full_refresh:
                entities = self._force_full_refresh_entities(entities)
    ...
    @staticmethod
    def _force_full_refresh_entities(entities: List[Entity]) -> List[Entity]:
        """Return per-run entity clones that ignore persisted watermarks.

        This is intentionally non-mutating: the control-plane watermark table is
        not edited. It is used for scoped smoke/proof runs where we need to seed
        Landing/Bronze/Silver artifacts even when the incremental watermark is
        already current.
        """
        refreshed: list[Entity] = []
        changed = 0
        for entity in entities:
            if entity.is_incremental or entity.last_load_value:
                changed += 1
                refreshed.append(replace(entity, is_incremental=False, last_load_value=None))
            else:
                refreshed.append(entity)
        if changed:
            log.warning(
                "Force full refresh enabled for %d/%d scoped entities; persisted watermarks are unchanged.",
                changed,
                len(entities),
            )
        return refreshed
