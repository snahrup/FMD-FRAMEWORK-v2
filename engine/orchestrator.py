...
from engine.runtime_v2.gating import scope_for_bronze, scope_for_silver
from engine.runtime_v2.models import TaskOutcome
...
class LoadOrchestrator:
    ...
    def __init__(self, config: EngineConfig):
        ...
        self._control_poll_interval_seconds: int = 2
    ...
    def run(
        ...
        resume_from_run_id : optional
            Source run whose successful work should be treated as already done
            while writing new progress into run_id.
        ...
        """
        resume_source_run_id = resume_from_run_id or resume_run_id

        # Determine current run_id and optional resume source lineage.
        if resume_source_run_id:
            run_id = run_id or resume_source_run_id
            self._resume_mode = True
            self._resume_source_run_id = resume_source_run_id
            if run_id == resume_source_run_id:
                log.info("RESUME MODE: resuming interrupted run %s", run_id)
            else:
                log.info(
                    "RESUME MODE: resuming source run %s into child run %s",
                    resume_source_run_id, run_id,
                )
        else:
            run_id = run_id or str(uuid.uuid4())
            self._resume_mode = False
            self._resume_source_run_id = None
        ...
        control = threading.Thread(
            target=self._control_loop, args=(run_id,),
            daemon=True, name="run-control",
        )
        control.start()
        ...
        try:
            ...
            if not layers or "bronze" in layers:
                ...
                if effective_method == "local":
                    bronze_scope = self._resolve_bronze_scope(run_id)
                    if bronze_scope.blocked_entity_ids:
                        log.warning(
                            "[%s] Bronze scope gated: %d allowed, %d blocked by landing failures/missing current-run output",
                            run_id[:8], len(bronze_scope.entity_ids), len(bronze_scope.blocked_entity_ids),
                        )
                    bronze_results = self._run_bronze_local(
                        run_id,
                        entity_ids=list(bronze_scope.entity_ids),
                    ) if bronze_scope.entity_ids else []
            ...
            if not layers or "silver" in layers:
                ...
                silver_t = time.time()
                if effective_method == "local":
                    silver_scope = self._resolve_silver_scope(run_id)
                    if silver_scope.blocked_entity_ids:
                        log.warning(
                            "[%s] Silver scope gated: %d allowed, %d blocked by bronze failures/missing current-run output",
                            run_id[:8], len(silver_scope.entity_ids), len(silver_scope.blocked_entity_ids),
                        )
                    silver_results = self._run_silver_local(
                        run_id,
                        entity_ids=list(silver_scope.entity_ids),
                    ) if silver_scope.entity_ids else []
            ...
            if self._stop_requested or self._run_abort_requested(run_id):
                log.warning("[%s] Run stopped before normal completion; preserving existing abort state", run_id[:8])
                self.status = "idle"
                return results
    ...
    def _get_completed_entity_ids(self, run_id: str, layer: str) -> set[int]:
        """Query engine_task_log for entities already succeeded in the active lineage."""
        completed: set[int] = set()
        for lineage_run_id in self._lineage_run_ids(run_id):
            rows = self._cpdb_query(
                "SELECT EntityId FROM engine_task_log "
                "WHERE RunId = ? AND LOWER(Layer) = LOWER(?) AND LOWER(Status) = 'succeeded'",
                (lineage_run_id, layer),
            )
            completed.update(int(r["EntityId"]) for r in rows)
        return completed

    def _lineage_run_ids(self, run_id: str) -> list[str]:
        lineage: list[str] = []
        if self._resume_source_run_id and self._resume_source_run_id != run_id:
            lineage.append(self._resume_source_run_id)
        lineage.append(run_id)
        return lineage

    def _get_task_outcomes(self, run_id: str, layer: str) -> list[TaskOutcome]:
        outcomes: list[TaskOutcome] = []
        for lineage_run_id in self._lineage_run_ids(run_id):
            rows = self._cpdb_query(
                "SELECT EntityId, Status, ErrorMessage "
                "FROM engine_task_log "
                "WHERE RunId = ? AND LOWER(Layer) = LOWER(?) "
                "ORDER BY id ASC",
                (lineage_run_id, layer),
            )
            for row in rows:
                ...
                outcomes.append(TaskOutcome(
                    ...
                    error=str(row.get("ErrorMessage") or "") or None,
                ))
        return outcomes

    def _get_run_db_status(self, run_id: str) -> str:
        rows = self._cpdb_query(
    ...
    def _run_abort_requested(self, run_id: str) -> bool:
        return self._get_run_db_status(run_id).lower() in {"aborted", "stopping", "interrupted"}

    def _resolve_bronze_scope(self, run_id: str):
        bronze_entities = self.get_bronze_worklist()
        landing_outcomes = self._get_task_outcomes(run_id, "landing")
        return scope_for_bronze(bronze_entities, landing_outcomes)

    def _resolve_silver_scope(self, run_id: str):
        silver_entities = self.get_silver_worklist()
        bronze_outcomes = self._get_task_outcomes(run_id, "bronze")
        return scope_for_silver(silver_entities, bronze_outcomes)
    ...
    def _control_loop(self, run_id: str) -> None:
        """Poll engine_runs for external stop/abort requests."""
        while self.status == "running" and not self._stop_requested:
            try:
                rows = self._cpdb_query(
                    "SELECT Status FROM engine_runs WHERE RunId = ?",
                ...
                if rows:
                    ...
                    if status in {"aborted", "stopping", "interrupted"}:
                        log.warning(
                            "[%s] External stop detected from control-plane status=%s",
                            run_id[:8], status,
                        )
                        self.stop(force=True)
                        break
            except Exception as exc:
                log.debug("[%s] Control-loop poll failed (non-fatal): %s", run_id[:8], exc)
