...
PACKETS = [
    ...
    # Visible gap closure batch
    {"id": "AUDIT-DL", "title": "Data Lineage audit + backend resilience", "prs": [40], "lane": "truth",
     "why": "6 findings: missing column_metadata table caused 500 (now cache miss), keyboard nav on clickable rows, a11y labels. 4 fixed, 2 deferred (shared layers.ts hex)."},
    {"id": "AUDIT-EM", "title": "Execution Matrix audit + display name fix", "prs": [41], "lane": "truth",
    ...
    # Frontend-only wave 3a
    {"id": "AUDIT-RC", "title": "Record Counts audit + false 100% fix", "prs": [51], "lane": "truth",
     "why": "8 findings: Math.round gave false 100% match rate, scan errors silently swallowed, hardcoded hex, a11y gaps. All 8 fixed, 0 deferred."},
    {"id": "AUDIT-IA", "title": "Impact Analysis audit + loading/error/keyboard", "prs": [52], "lane": "truth",
     "why": "8 findings: blank screen during load, error never rendered, search dropdown mouse-only, invalid Tailwind opacity syntax. All 8 fixed, 0 deferred (page-contained)."},
    {"id": "AUDIT-FE", "title": "Flow Explorer audit + nav fix + layer truth", "prs": [53], "lane": "truth",
...
PAGES = [
    ...
    {"name": "Execution Matrix", "route": "/matrix", "lane": "Truth/UI", "packets": ["UP-03", "AUDIT-EM"],
     "focus": "Audited — DONUT_COLORS race fix + OPTIVA display name + a11y", "next": "Done for now"},
    ...
    {"name": "Data Profiler", "route": "/profile", "lane": "Truth/UI", "packets": ["UP-02", "AUDIT-DP"],
     "focus": "Audited — CRITICAL row count truth fix + path traversal + token cleanup", "next": "Done for now"},
    ...
    {"name": "Flow Explorer", "route": "/flow", "lane": "Hidden/Audited", "packets": ["AUDIT-FE"],
    ...
    {"name": "Impact Analysis", "route": "/impact", "lane": "Hidden/Audited", "packets": ["AUDIT-IA"],
     "focus": "Audited — loading/error/empty states added, keyboard nav on search, all 8 fixed", "next": "Done for now"},
